...
<title>Create POS Shop — OWERU</title>
...
.form-group input, .form-group textarea {
  ...
  font-family: var(--f-sans); font-size: 14px; outline: none; transition: border-color 0.2s; background: var(--cream);
}
...
<div class="container">
  <div class="page">
    ...
    <p>Set up a new point-of-sale location for your business.</p>

    <form method="POST" action="{{ route('pos.shops.store') }}" class="form-card">
      @csrf
      <div class="form-group">
        <label>Shop Name *</label>
        <input type="text" name="name" placeholder="e.g. Downtown Store" required value="{{ old('name') }}">
      </div>
      <div class="form-group">
        <label>Location</label>
        <input type="text" name="location" placeholder="e.g. 123 Main Street, Dar es Salaam" value="{{ old('location') }}">
      </div>
      <div class="form-group">
        <label>Phone</label>
        <input type="text" name="phone" placeholder="+255..." value="{{ old('phone') }}">
      </div>
      <div class="form-group">
        <label>Email</label>
      ...
      <div class="actions">
        ...
        <button type="submit" class="btn btn--gold"><i class="fas fa-plus"></i> Create Shop</button>
      </div>
    </form>
  </div>
</div>
